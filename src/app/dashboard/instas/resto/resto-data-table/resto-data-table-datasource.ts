...
export interface RESTODataTableItem {
  name: string;
  id: number;
  adress : string;
  surface : number;
...
const EXAMPLE_DATA: RESTODataTableItem[] = [
...
export class CUDataTableDataSource extends DataSource<any> {
   ...
   */
  connect(): Observable<RESTODataTableItem[]> {
    const dataMutations = [
   ...
   */
  private getPagedData(data: RESTODataTableItem[]) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: RESTODataTableItem[]) {
